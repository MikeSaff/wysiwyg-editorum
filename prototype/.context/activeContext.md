...
| Роль | Статус | Задача |
|------|--------|--------|
| **Codex** | **IN PROGRESS** | **Формула (2): 4-я строка оторвана — см. CODEX-TASK-FORMULA-LINES.md** |
| Claude | DONE | Скобки cases — mfenced fix, shouldWrapFormulaLinesInCases=false |
| Claude | DONE | MathJax menu/explorer отключён |
| Composer | Свободен | Брать задачу из progress.md |

---

## Архитектура

- Стек: ProseMirror + Vite + MathJax
- Документ: DocumentJSON (ProseMirror JSON)
- Формулы: OMML → MathML → MathJax (v0.42+)
