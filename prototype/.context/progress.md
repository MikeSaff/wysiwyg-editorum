...
## Сессия 2026-04-28 — v0.59

| # | Задача | Кто | Файлы | Статус | Билд |
|---|--------|-----|-------|--------|------|
| v0.59 | UI acceptance suite + visual regression fixes. `v0.59 task 0:` Playwright installed successfully; fresh `vite preview` gate added, `ui-acceptance` now imports real Trukhachev and checks DOM after MathJax hydration. `v0.59 task 2 diagnosis:` причина не-работы TOC = старый position-based nav + paragraph heuristics + повторный scroll через ProseMirror selection после click; исправлено на structural-only anchors + deterministic scroll with sticky offset. `v0.59 task 4 diagnosis:` на Trukhachev rendered DOM уже имел `3 figure-block + 1 loose image`; визуальный баг давал не merge figure candidates, а surviving backmatter tail `ПОДПИСИ К РИСУНКАМ` + stray `Рис. 4.` в body; caption-list tail теперь вычищается после figure-caption expansion. `v0.59 task 5 diagnosis:` UDK input был привязан к `meta.udk`, но panel read/write обходил только `.md-inp[data-k]`; исправлено на `.md-inp` + `.md-inp-sm`, publication date зеркалится в canonical `meta.publicationDate`. | Codex | `prototype/playwright.config.js`, `prototype/tests-e2e/*`, `prototype/src/main.js`, `prototype/src/navigation.js`, `prototype/src/section-heading.js`, `prototype/src/schema.js`, `prototype/src/styles.css`, `prototype/src/word-import.js`, `prototype/src/metadata-panel.js`, `prototype/src/metadata-extract.js`, `prototype/scripts/formula-quality-lib.mjs`, `prototype/tests/*`, `.gitignore`, `.context/*` | ✅ | `mtef npm run build && npm test` OK; `prototype npm install && npm test && npx vite build` OK; `corpus:baseline`, `corpus:diff`, `formula-quality`, `ui-acceptance` OK; Trukhachev `figure_count=3`, `figure_caption_gap_count=0`, `section_type_other_count=0`, `UDK=533.9` |

## Сессия 2026-04-28 — v0.60

| # | Задача | Кто | Файлы | Статус | Билд |
|---|--------|-----|-------|--------|------|
| v0.60 | Direction-aware figure pairing. `v0.60 task 1 diagnosis:` subsequent captions на Trukhachev: `image93=pi111 (Рис. 1.)`, `image94=pi114 (Рис. 2.)`, `image95=pi116 (Рис. 3.)`, `image96=pi120 (Рис. 4.)`; preceding-direction давал shift `-1`. Fix: image-only paragraphs ищут captions subsequent-first через empty spacers, preceding остаётся fallback, consumed captions не переиспользуются. Full `pStyle=Figure` caption text прикрепляется tail-aligned к последним short markers; invalid/empty images dropped. | Codex | `prototype/src/word-import.js`, `prototype/scripts/formula-diff.mjs`, `prototype/tests/word-import.test.js`, `prototype/tests-e2e/trukhachev.spec.js`, `prototype/tests/corpus-baseline.json`, `prototype/tests/formula-quality-baseline.json`, `.context/*` | ✅ | `mtef npm run build && npm test` OK; `prototype npm install && npm test && npx vite build` OK; `corpus:baseline`, `corpus:diff`, `formula-quality`, `ui-acceptance` OK; Trukhachev `figure_count=4`, `figure_caption_gap_count=0`, `0 loose images`, captions `Рис. 1`…`Рис. 4` |
| v0.61 | Composite script target resolution in MTEF. `v0.61 task 1 diagnosis:` real OLE `(19)` `oleObject66.bin` has postfix `tmSUB` at pos `244/375/479` and postfix `tmSUBSUP` at pos `413`; real OLE `(20)` `oleObject67.bin` has postfix `tmSUB` at pos `210/254/400/504` and postfix `tmSUBSUP` at pos `438`. These selector `27/29` records have empty base slots and target the previous plain atom, so the old renderer flattened `υᵢ` to `\upsilon i` and `υᵢ²` to `\upsilon i_{2}`. Parser row normalization now converts postfix scripts to semantic `msub`/`msubsup`; LaTeX emits `\upsilon_{i}` and `\upsilon_{i}^{2}`. | Codex | `mtef-to-mathml/src/parser.ts`, `mtef-to-mathml/src/latex.ts`, `mtef-to-mathml/test/parser.test.ts`, `mtef-to-mathml/test/golden/*trukhachev-velocity*`, `prototype/tests-e2e/trukhachev.spec.js`, `prototype/tests/corpus-baseline.json`, `prototype/tests/formula-quality-baseline.json`, `.context/*` | ✅ | `mtef npm run build && npm test` OK (`86 passed`); `prototype npm install && npm test && npx vite build` OK (`143 tests`, 1 existing skip); `corpus:baseline`, `corpus:diff`, `formula-quality`, `ui-acceptance` OK; Trukhachev formulas `(19)/(20)` contain grouped `\upsilon_{i}` and `\upsilon_{i}^{2}`, v0.58 accents and v0.60 figures remain green |
